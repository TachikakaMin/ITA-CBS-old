...
class NextBestAssignment {
 public:
  ...
  std::map<std::pair<Agent, Task>, long> getCostMatrix()
  {
    return m_cost;
  }
...
//     std::endl;
  ...
  // find next solution
  long nextSolution(std::map<Agent, Task>& out_solution) {
    out_solution.clear();
    if (m_open.empty()) {
    ...
    for (const auto& entry : next.solution) {
      out_solution.insert(std::make_pair(entry.first, entry.second));
    }
    ...
    for (size_t i = 0; i < m_agentsVec.size(); ++i) {
      if (fixedAgents.find(m_agentsVec[i]) == fixedAgents.end()) {
        ...
        for (size_t j = 0; j < i; ++j) {
          ...
          const auto iter = out_solution.find(agent);
          if (iter != out_solution.end()) {
        ...
        const auto iter = out_solution.find(m_agentsVec[i]);
        if (iter != out_solution.end()) {
  ...
  long newCostMatrixAssignment(std::map<std::pair<size_t, Task>, long>& cost_matrix,
                               std::map<Agent, Task>& out_solution)
  {

     long cost = constrainedMatching2(cost_matrix, out_solution);
     return cost;
  }
 ...
 protected:
  ...
  long constrainedMatching(const std::set<std::pair<Agent, Task> >& I,
    ...
    m_assignment.solve(out_solution);

    size_t matching = numMatching(out_solution);

    // check if all agents in Iagents have an assignment as requested
    bool solutionValid = true;
    for (const auto& agent : Iagents) {
      if (out_solution.find(agent) == out_solution.end()) {
        solutionValid = false;
        break;
      }
    }
    ...
    for (const auto& c : I) {
      const auto& iter = out_solution.find(c.first);
      if (iter == out_solution.end()
    ...
    return cost(out_solution);
  }

  long constrainedMatching2(std::map<std::pair<size_t, Task>, long>& cost_matrix, std::map<Agent, Task>& out_solution) {
    // prepare assignment problem

    m_assignment.clear();

//    std::vector<Task> all_goals;
//    for (auto [key, value]: cost_matrix) {
//        all_goals.push_back(key.second);
//    }
//    std::sort(all_goals.begin(), all_goals.end());
//    for (auto goal: all_goals)
//        m_assignment.setCost(0, goal, 1e9);
//    for (int i=0; i< m_numMatching;i++)
//        m_assignment.setCost(i, all_goals[0], 1e9);

    for (auto [key, value]: cost_matrix) {
      m_assignment.setCost(key.first, key.second, value);
      this->setCost(key.first, key.second, value);
    }

    m_assignment.solve(out_solution);
    size_t matching = numMatching(out_solution);

    if (matching < m_numMatching) {
      out_solution.clear();
      return std::numeric_limits<long>::max();
    }
    return cost(out_solution);
