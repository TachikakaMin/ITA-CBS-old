...
class AStar {
 public:
  ...
  bool search(const State& startState,
    ...
    solution.states.push_back(std::move(std::make_pair<>(startState, 0)));
    solution.actions.clear();
    solution.cost = 0;
    int search_max = 1e4;
    openSet.clear(); // d_ary_heap<Node>
    stateToHeap.clear();
    closedSet.clear();
    cameFrom.clear();
    auto handle = openSet.push(
        std::move(Node(startState, m_env.admissibleHeuristic(startState), initialCost)));
    stateToHeap.insert(std::move(std::make_pair<>(startState, handle)));
      ...
      for (const Neighbor<State, Action, Cost>& neighbor : neighbors) {
        if (closedSet.find(neighbor.state) == closedSet.end()) {
          ...
          if (iter == stateToHeap.end()) {  // Discover a new node
            ...
            auto handle =
                openSet.push(std::move(Node(neighbor.state, fScore, tentative_gScore)));
            (*handle).handle = handle;
            stateToHeap.insert(std::move(std::make_pair<>(neighbor.state, handle)));
          ...
          cameFrom.insert(std::move(std::make_pair<>(
              neighbor.state,
