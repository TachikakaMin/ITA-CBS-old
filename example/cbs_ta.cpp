...
//    std::cout<<m_numTaskAssignments<<std::endl;
...
int main(int argc, char *argv[]) {
    ...
    std::unordered_set < Location > single_goals;
...
//    if (node["goal"].size() > 0)
//      single_goals.emplace(Location(node["goal"][0].as<int>(), node["goal"][1].as<int>()));
    }

//  int tmpi = 0;
//  for (const auto& node : config["agents"]) {
//    if (node["goal"].size() > 0){
//      goals[tmpi] = single_goals;
//    }
//    tmpi ++;
//  }
