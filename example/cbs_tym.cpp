...
#include <libMultiRobotPlanning/dynamic_hungarian_assignment.hpp>
...
using libMultiRobotPlanning::DynamicHungarianAssignment;
...
//
//  void remove(const Constraints& other) {
//    vertexConstraints.erase(vertexConstraints.find(10));
//  }
...
public:
    Environment(size_t dimx, size_t dimy,
        ...
        m_agents_goals = goals;
        new_assignment.initHungarian(startStates, goals);

        std::vector<Location> all_goals;
        ...
        for (size_t i = 0; i < startStates.size(); ++i)
            for (auto goal: all_goals)
                m_assignment.setCost(i, goal, 1e9);
...
//        new_assignment.solve();
    ...
    void nextCostMatrixAssignment(std::map<std::pair<size_t, Location>, long> &cost_matrix,
                                  std::map<size_t, Location> &out_tasks) {
        if (m_numTaskAssignments > m_maxTaskAssignments) {
            return;
        }
        int64_t cost = m_assignment.newCostMatrixAssignment(cost_matrix, out_tasks);
        if (!out_tasks.empty()) {
...
//    void newFirstTaskAssignment(HighLevelNode& start)
//    {
//        if (m_numTaskAssignments > m_maxTaskAssignments) {
//            return;
//        }
//
//        int64_t cost = start.new_assignment.firstSolution(start.cost_matrix, start.out_tasks);
//        if (!start.out_tasks.empty()) {
...
//    void incrementalTaskAssignmentX(const std::map<std::pair<size_t, Location>, long>& cost_matrix,
//                                   std::map<size_t, Location>& out_tasks,
//                                   size_t u)
//    {
//        if (m_numTaskAssignments > m_maxTaskAssignments) {
//            return;
//        }
//        int64_t cost = new_assignment.incrementalSolutionX(cost_matrix, out_tasks, u);
//        if (!out_tasks.empty()) {
//            ++m_numTaskAssignments;
//        }
//    }

    void nextTaskAssignment(std::map<size_t, Location> &out_tasks) {
        if (m_numTaskAssignments > m_maxTaskAssignments) {
            return;
        }


        int64_t cost = m_assignment.nextSolution(out_tasks);
        if (!out_tasks.empty()) {
    ...
    NextBestAssignment<size_t, Location> getMAssignment() {
        return m_assignment;
    }
...
public:
    ...
    std::vector<std::unordered_set<Location> > m_agents_goals;
    DynamicHungarianAssignment<size_t, Location> new_assignment;
...
int main(int argc, char *argv[]) {
    ...
    timer.reset();
    std::cout << "maxTaskAssignments: " << maxTaskAssignments << std::endl;
    Environment mapf(dimx, dimy, obstacles, startStates, goals,
                     maxTaskAssignments);
    timer.stop();
    std::cout << "  Environment build: " << timer.elapsedSeconds() << std::endl;
    ...
    if (success) {
        ...
        out << "  TA_runtime: " << cbs.p_ta_time << std::endl;
        out << "  lowlevel_search_time: " << cbs.p_lowlevel_search_time << std::endl;
        out << "  lowlevel_queue_time: " << cbs.p_lowlevel_queue_time << std::endl;
        out << "  create_low_level_node_time: " << cbs.p_create_low_level_node_time << std::endl;
        out << "  TA_gen_time: " << cbs.avg_next_ta_time << std::endl;
        out << "  d_cost: " << cbs.d_cost << std::endl;

        out << "  runtime: " << timer.elapsedSeconds() << std::endl;
        out << "  total_lowlevel_node: " << cbs.id << std::endl;
